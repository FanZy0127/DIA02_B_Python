...
class MappingSubclass(Mapping):
    ...
    def update(self):
        for item in self.iterable:
            self.items_list.append(item.upper())

mapMother = Mapping("Bonjour")
...
Ecrire une classe HasCap qui renvoie tous les mots/particules d'un texte t commençant par une majuscule dans un dictionnaire, en comptant le nombre d'occurence de ces mots/particules. Les occurences sont insensible à la classe.
